...
import { acquisitionSourcesData, COLORS } from "../../../data/dashboardData";
import { getTooltipStyles } from "../../../utils/darkModeUtils";

export default function AcquisitionSources({ timeRange, darkMode }) {
    ...
    <div className="bg-white dark:bg-dark p-6 rounded-xl shadow-sm border border-slate-100 dark:border-gray-700">
      ...
      <div className="w-full h-[300px] bg-white dark:bg-dark">
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie
              data={acquisitionSourcesData[timeRange]}
              cx="50%"
              cy="50%"
              labelLine={false}
              outerRadius={80}
            ...
            >
              {acquisitionSourcesData[timeRange].map((_, index) => (
                <Cell
                  key={`cell-${index}`}
